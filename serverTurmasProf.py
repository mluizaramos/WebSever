...
class MyHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/turmasProfessor":
            try:
                with open(os.path.join(os.getcwd(), "TurmaDoProfessor.html"), "r", encoding="utf-8") as login_file:
    ...
    def usuario_existente(self, login, turma):
    ...
    def do_POST(self):
        # Verificar se a rota é "/confirmarTurma"
        if self.path == "/confirmarTurma":
            ...
            if self.usuario_existente(login, turma):
            ...
            # Se não for a rota "/confirmarTurma", continuar com o comportamento padrão
            super().do_POST()
